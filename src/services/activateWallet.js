...
export async function activateWallet(
    ownerPublicKey, walletOwnerAddress) {
    ...
    let activateUserOperation = initializedSoulWalletLib.activateWalletOp(
        ...
        ethers.utils
          .parseUnits(constants.CONFIG.MOCK_GAS_FEES.high.suggestedMaxFeePerGas, "gwei")
          .toString(),
        ethers.utils
          .parseUnits(constants.CONFIG.MOCK_GAS_FEES.high.suggestedMaxPriorityFeePerGas, "gwei")
          .toString(), 
    ...
    const bundlerEvent = bundler.sendUserOperation(activateUserOperation, 1000 * 60 * 3);
...
async function calculateActivateUserOperationCallData(
    ...
    return await initializedSoulWalletLib.Tokens.ERC20.getApproveCallData(provider, walletAddress, approveData);
...
async function calculateUserOperationPaymasterAndData(
    ...
    const requiredPreFundObtained = userOperation.requiredPrefund(
      ethers.utils.parseUnits(constants.CONFIG.MOCK_GAS_FEES.estimatedBaseFee, "gwei"));
    ...
    const exchangePriceObtained = await initializedSoulWalletLib.getPaymasterExchangePrice(
      provider, constants.CONFIG.TOKENPAYMASTER_CONTRACT_ADDRESS, constants.CONFIG.USDC_CONTRACT_ADDRESS, true);
    ...
    return initializedSoulWalletLib.getPaymasterData(constants.CONFIG.TOKENPAYMASTER_CONTRACT_ADDRESS, 
        constants.CONFIG.USDC_CONTRACT_ADDRESS, maxUSDC);
  }
